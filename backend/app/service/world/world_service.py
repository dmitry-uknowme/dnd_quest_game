...
from api.world.schemas.world import CreateWorldAgentResponseSchema
from service.world.schemas import WorldResponseSchema


async def get_world(db: AsyncSession, world_id: str) -> World | None:
    return await world_repository.get_world(db, world_id)

async def create_ai_world_by_title(db: AsyncSession, title: str) -> World:
